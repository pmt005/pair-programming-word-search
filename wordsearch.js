const wordSearch = (letterMatrix, word) => {
  //This will confirm input is an array and also confirm matrix is not empty
  if ((!Array.isArray(letterMatrix)) || letterMatrix.length === 0) {
    return false;
  }

  // This will confirm input is a MATRIX
  for (let element of letterMatrix) {
    if (element.length === 0) {
      return false;
    }
  }
